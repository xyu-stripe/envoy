...
RetryPolicyImpl::RetryPolicyImpl(const envoy::api::v2::route::RetryPolicy& retry_policy,
  ...
  // NB(xyu-stripe): parseRetryOn and parseRetryGrpcOn will return a value with
  // RETRY_UNKNOWN_FIELD_ERROR set unless the input only contains supported
  // values for x-envoy-retry-on and x-envoy-retry-grpc-on, respectively. Clear
  // this flag here, since the `retry_on` config option is intended to contain
  // retry policies from both.
